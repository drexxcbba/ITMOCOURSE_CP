...
struct segtree{
  ...
  int find(int k){
   ...
   int firstAbove(int x, int curr, int lx, int rx){
     if(values[curr] < x) return -1;
     if(rx - lx == 1) return lx;     
     int m = (lx + rx) / 2;
     int res = firstAbove(x, 2 * curr + 1, lx, m);
     if(res == -1) res = firstAbove(x, 2 * curr + 2, m, rx);
     return res;
   }

  int firstAbove(int x){
    return firstAbove(x, 0, 0, size);
  }
};
