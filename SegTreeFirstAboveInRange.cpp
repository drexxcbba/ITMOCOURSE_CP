...
struct segtree{
  ...
  int firstAboveRange(int x, int l, int curr, int lx, int rx){
    if(values[curr] < x) return -1;
    if(rx <= l) return -1;
    if(rx - lx == 1) return lx;     
    int m = (lx + rx) / 2;
    int res = firstAboveRange(x, l, 2 * curr + 1, lx, m);
    if(res == -1) res = firstAboveRange(x, l, 2 * curr + 2, m, rx);
    return res;
  }

  int firstAboveRange(int x, int l){
    return firstAboveRange(x, l, 0, 0, size);
  }
};
...
int main() {
  ...
  while (m--){
    ...
    if(q == 1){
      ...
      int x, l;
      cin>>x>>l;
      cout<<st.firstAboveRange(x, l)<<endl;
    }
  }
  return 0;
}